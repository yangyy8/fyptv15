<template>
    <div class="pairadd">
         <div class="homebread"><i class="iconfont el-icon-yy-mianbaoxie" style="color:#3872A2"></i><span> 联络工作
            <span class="mlr_10">/</span>  <b>{{cname}}</b></span> </div>
               ...
               <div class="pborder mb-50">
                   <el-row class="ah-40">
                     <el-col :span="12" class="input-item" v-if='addtype=="4" || addtype=="5"'>
                      <span class="yy-input-text" style="width:22.5%;"><font class="red">*</font> 时间</span>
                     ...
                     <el-col :span="24" v-if="addtype!=9 && addtype!=11" class="input-item">
                      ...
                      <el-col :span="8"  v-if="addtype!=9 && addtype!=11" class="input-item">
                       ...
                       <el-col :span="8"  v-if="addtype!=9 && addtype!=11" class="input-item">
                            ...
                            <el-select v-model="form.devDepartmentId"   :disabled="llbnt" filterable clearable default-first-option placeholder="请选择"  size="small" class="yy-input-input" :no-data-text="form.developmentUnitId==''||form.developmentUnitId==undefined?'请先选择开展单位':'无数据'">
                               <el-option
                       ...
                       <el-col :span="8"  v-if="addtype==9" class="input-item">
                            ...
                            <el-select v-model="form.receptionDepartmentId"  @change="getJDR(form.receptionUnitId,form.receptionDepartmentId)" :disabled="llbnt" filterable clearable default-first-option placeholder="请选择"  size="small" class="yy-input-input" :no-data-text="form.receptionUnitId==''||form.receptionUnitId==undefined?'请先选择接待单位':'无数据'">
                               <el-option
                   ...
                   <el-row class="ah-40"  v-if="addtype==6 || addtype==5 || addtype==7 || addtype==4">
                       ...
                       <el-col :span="8" class="input-item">
                            ...
                            <el-select v-model="form.undertakeDepartmentId" :disabled="llbnt" filterable clearable default-first-option placeholder="请选择"  size="small" class="yy-input-input" :no-data-text="form.undertakeUnitId==''||form.undertakeUnitId==undefined?'请先选择承办单位':'无数据'">
                               <el-option
                    ...
                    <el-row  class="mt-20" v-if='addtype=="7"'>
                        <el-col :span="12"  class="input-item">
                            ...
                            <!-- <el-select v-model="fymber" remote :remote-method="fyrdwremoteMethod" v-el-select-loadmore="fyrloadmore" @visible-change="getFYName($event)" v-if="!llbnt" filterable clearable default-first-option placeholder="请选择"  size="small" class="yy-input-input" style="width:70%!important" >
                             ...
                             <el-select v-model="fymber" remote :remote-method="fyrdwremoteMethodnew" allow-create v-el-select-loadmore="fyrloadmorenew" @focus="getfocus(1)"  @change="getNullVlaue(fymber,2)"   v-if="!llbnt" filterable  clearable default-first-option placeholder="请输入关键字搜索"  size="small" class="yy-input-input" style="width:70%!important" >
                               <el-option
                                 v-for="(item,ind) in fydata"
                                 :key="ind"
                                 :label="item.fullName"
                  ...
                  <el-row>
                    <el-col :span="24"  class="input-item">
                      ...
                      <el-col :span="8" class="input-item">
                            ...
                            <el-select v-model="fyld" remote :remote-method="fyrdwremoteMethodnew" v-el-select-loadmore="fyrloadmorenew" @focus="getfocus(1)"   @change="getNullVlaue(fyld,2)"   v-if="!llbnt" filterable  clearable allow-create default-first-option placeholder="请输入关键字搜索"  size="small" class="yy-input-input"  >
                               <el-option
                                 v-for="(item,ind) in fydata"
                                 :key="ind"
                                 :label="item.fullName"
                      ...
                      <el-col :span="8"  class="input-item">
                        <span class="yy-input-text textn">
                            ...
                            <el-select v-model="ssxx.orgId" remote :remote-method="fydwremoteMethod" v-el-select-loadmore="fyloadmore" @focus="getfocus(3)"   :disabled="llbnt" @change="getcbbm(ssxx.orgId,4)"  filterable clearable allow-create default-first-option placeholder="请输入关键字搜索"  size="small" class="yy-input-input" >
                               <el-option
                                 v-for="(item,ind) in fydwdata"
                                 :key="ind"
                                 :label="item.mc"
                      ...
                      <el-col :span="8"  class="input-item">
                        <span class="yy-input-text textn">
                            ...
                            <el-select v-model="ssxx.subOrgId" :disabled="llbnt" filterable clearable allow-create default-first-option placeholder="请选择"  size="small" class="yy-input-input" :no-data-text="ssxx.orgId?'无数据':'请先选择所属单位'">
                               <el-option
                      ...
                      <el-col :span="8"  class="input-item">
                        <span class="yy-input-text textn">
                            ...
                            <el-select v-model="ssxx.position" :disabled="llbnt" filterable clearable allow-create default-first-option placeholder="请选择"  size="small" class="yy-input-input" >
                               <el-option
...
export default {
    ...
    data(){
        return{
          ...
          pd3:{},
          pd4:{},//批示列表
          pd5:{ck1:false,ck2:false,
          ck3:false,ck4:false,ck5:false,
          ck7:false,ck6:false,ck8:false,
          ck9:false,ck10:false,ck11:false},//复选框 单元框
          pd6:{},
          pd7:{},//接待人员列表
          pd8:{},//关注案件
          pd10:{},//用于日常建议
  ...
  watch: {
    ...
    methods:{
       ...
       getfykeyWord(name){
          let p={
              ...
              "itemIndex": [
                "API_PERSON_SUCCESSION"
              ]
            }
           this.$api.post('http://192.168.50.111:9406/risk/analysis',p,
                r =>{
                    if(r.code==200){
    ...
    //     },
        ...
        reset(){
            ...
            this.pd7={};
            this.pd8={};
            this.pd5={ck1:false,ck2:false,ck3:false,
            ck4:false,ck5:false,ck7:false,
            ck6:false,ck8:false,ck9:false,ck10:false,ck11:false},//复选框 单元框
                ...
                r =>{
                     ...
                     if(r.code==1){
                        ...
                        //开展单位
                         ...
                         if(this.addtype=='4' || this.addtype=='5' || this.addtype=='6' || this.addtype=='7')
                          {
             ...
             if(t==1){ //视察列表
                 ...
                 this.fyquery();
                  ...
                  if(this.pd3.newsMediaName==undefined || this.pd3.newsMediaName==''){
                     this.$message.error("首发新闻单位不能为空!");return;
                 }
                 if(this.pd3.newsMediaTypeId==undefined || this.pd3.newsMediaTypeId==''){
                     this.$message.error("宣传报道形式不能为空!");return;
                 }
        ...
        scdwremoteMethod(quer){
          if (quer !='') {
            let p={
             ...
             this.$api.post(this.Global.aport1+'/org/getVisitOrg',p,
                  r =>{
                      if(r.code==1){
                          this.scdwload=r.data;
                           if(this.scdwload.length>this.jznum){
      ...
      getCBDW(){
        ...
        getyj(t){
          if(t!=0){
            ...
            if(this.ListData4.length==0){
               this.$message.error('请先选择代表、委员以及特约人员！');return;
             }
          }
        ...
        getwx(){
         if(this.ListData4  && this.ListData4.length>0){
             ...
             this.$nextTick(function () {
                 ...
                 if(this.pd5.ck11){ this.$refs.mlTable.toggleRowSelection(this.ListDataWX[i],true);}
                }
              })
         }else{
             this.$message.error("请先选择代表、委员以及特约人员！");  this.$set(this.pd5,'ck11',false);return;
         }
