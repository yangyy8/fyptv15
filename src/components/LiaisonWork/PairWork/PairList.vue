...
<template>
    <div class="pairadd indexList">
         ...
         <div class="content subtable">
                ...
                <div class="pborder">
                    ...
                    <el-row class="lh" v-if="open">
                        ...
                        <el-select v-model="pd.courtPersonIdArr" @focus="getfocus(2)" remote :remote-method="fyrdwremoteMethodnew" v-el-select-loadmore="fyrloadmorenew" multiple filterable clearable default-first-option placeholder="请输入关键字搜索"  size="small" class="yy-input-input"  >
                               <el-option
                                 v-for="(item,ind) in fydata"
                                 :key="ind"
                                 :label="item.fullName"
         ...
         <el-dialog title="选择类型" :visible.sync="addDialogVisible" :close-on-click-modal='false' width="700px">
 ...
 <el-dialog title="导入文件" :visible.sync="uploadDialogVisible" :close-on-click-modal='false'  width="640px">
  ...
  <el-dialog title="内容发布" :visible.sync="fbDialogVisible" :close-on-click-modal='false'  width="880px">
...
export default {
    ...
    methods:{
        ...
        getCK(t){
         ...
         if(types=='1'){
            this.$router.push({name:'PairAdd',query:{type:types,state:t,activityInfoId:this.multipleSelection[0].activityInfoId}});

         }else{
            this.$router.push({name:'SpecialAdd',query:{type:types,state:t,activityInfoId:this.multipleSelection[0].activityInfoId}});
         }
        },
        goadd(){
            ...
            if(this.addtype==1){
            this.$router.push({name:'PairAdd',query:{type:this.addtype,state:this.status}});
            }else{
            this.$router.push({name:'SpecialAdd',query:{type:this.addtype,state:this.status}});
            }
        },
        reset(){
            this.pd={};
        ...
        getList(currentPage, showCount, pd){
          ...
          this.$api.post(this.Global.aport2+'/ActivityInfoController/queryActivityInfo',p,
                r =>{
        ...
        delpair(){
          ...
          download(t){
            ...
            this.$api.post(this.Global.aport2+'/ActivityInfoController/exportActivityInfo',p,
                r =>{
                      this.downloadM(r);
                },e=>{},{},'blob')
          },
        ...
        getopen(t){
            ...
            for (let i = 0; i < array.length; i++) {
                ...
                if(array[i].activityTypeId=="0149000001"){
                    this.$message.error("其中的结对活动不操作公开，审核，发布，回收！");  return;
                }
                var obj={};
